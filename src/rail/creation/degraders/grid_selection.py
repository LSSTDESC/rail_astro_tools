...
class GridSelection(Selector):
    ...
    config_options.update(
        ...
        percentile_cut=Param(
            float,
            99.0,
            msg="percentile cut-off for each pixel in color-based redshift cut off",
        ),
        ...
        scaling_factor=Param(
            float,
            1.588,
            msg="multiplicative factor for ratios to adjust number of galaxies kept",
        ),
    )
    ...
    def _select(self):
        ...
        for val in check_vals:
            if val not in settings.keys():  # pragma: no cover
                raise KeyError(
                    f"required key {val} not present in {self.config.settings_file}!"
                )
        for val in check_vals[:-2]:
            if (
                settings[val] != "" and settings[val] not in data.keys()
            ):  # pragma: no cover
        ...
        if settings["x_band_2"] == "":
            ...
            data["x_vals"] = (
                data[settings["x_band_1"]] - data[settings["x_band_2]"]]
            ).to_numpy()
        ...
        if settings["y_band_2"] == "":  # pragma: no cover
            ...
            data["y_vals"] = (
                data[settings["y_band_1"]] - data[settings["y_band_2"]]
            ).to_numpy()
        ...
        if not self.config.color_redshift_cut:  # pragma: no cover
            max_specz = np.ones_like(ratios) * 100.0
        else:
            ...
            hsc_spec_galaxies["total_pixel"] = (
                pixel_tot  # tags each galaxy with a single pixel number instead of one color and one magnitude
            )
            ...
            for i in range(len(x_edges) - 1):
                for j in range(len(y_edges) - 1):
                    ...
                    if np.isin(pixel, unique_pixels):
                        temp_dict = hsc_spec_galaxies[
                            hsc_spec_galaxies["total_pixel"] == pixel
                        ]
        ...
        data_hsc_like_redshift_cut = data_hsc_like[
            data_hsc_like["redshift"] <= data_hsc_like["max_specz"]
        ]

        # If making a redshift cut, do that now
        if self.config["redshift_cut"] != 100:  # pragma: no cover
            data_hsc_like_redshift_cut = data_hsc_like_redshift_cut[
                data_hsc_like_redshift_cut["redshift"] <= self.config["redshift_cut"]
            ]
        ...
        for xratio in unique_ratios:
            temp_data = data_hsc_like_redshift_cut[
                data_hsc_like_redshift_cut["ratios"] == xratio
            ]
