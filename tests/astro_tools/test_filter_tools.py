...
from rail.tools.filter_tools import (
    calc_lambda_effective_from_arrays,
    calc_lambda_effective_from_file,
    ccm_alam_over_ebv,
)


def test_wl_from_file():
    gfiltpath = os.path.join(
        RAILDIR, "rail/examples_data/estimation_data/data/FILTER/DC2LSST_g.res"
    )
    leff = calc_lambda_effective_from_file(gfiltpath)
    assert np.isclose(leff, 4826.8517, atol=1.0e-2)
    alam = ccm_alam_over_ebv(leff)
    assert np.isclose(alam, 3.64287, atol=1.0e-2)


def test_wl_from_dummy_data():
    dummywl = np.linspace(3000, 6000, 301)
    tr = np.exp(-1.0 * (dummywl - 4500.0) ** 2 / (2.0 * 900.0))
    leff = calc_lambda_effective_from_arrays(dummywl, tr)
    assert np.isclose(leff, 4500.0, atol=1.0e-2)
    # alamba for 4500 should be 3.9925
    alam = ccm_alam_over_ebv(leff)
    assert np.isclose(alam, 3.9925, atol=1.0e-2)
...
def test_specific_wls():
    ...
    for wl, true_alam in zip(wllist, alams):
        ...
        assert np.isclose(alam, true_alam, atol=1.0e-2)
