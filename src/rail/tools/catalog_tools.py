...
class CatalogManipulator(RailStage, ABC):
    ...
    config_options.update(
        major_columns=Param(
            str, default="major", msg="column names for semi-major axes."
        ),
        minor_columns=Param(
            str, default="minor", msg="column names for semi-major axes."
        ),
        to_arcsec=Param(
            bool,
            default=False,
        ...
        arcsec_per_pix=Param(
            float, default=ARCSEC_PER_PIX, msg="Size of the pixel in arcsec."
        ),
    )
...
class MajorEllipticityToAB(CatalogManipulator):
    ...
    config_options.update(
        ...
        ellipticity_column=Param(
            str, default="ellipticity", msg="column names for ellipticity"
        ),
    )
    inputs = [("input", PqHandle)]
    outputs = [("output", PqHandle)]
...
class BulgeDiscSizeEllipticityToAB(CatalogManipulator):
    ...
    config_options = CatalogManipulator.config_options.copy()
    config_options.update(
        bulge_size_column=Param(str, default="size_bulge_true", msg="Bulge size"),
        disk_size_column=Param(str, default="size_disk_true", msg=""),
        bulge_to_total_ratio_colum=Param(
            str,
            default="bulge_to_total_ratio",
            msg="column names for size (here defined to be the geometric average of semi-major and minor axes)",
        ),
        ellipticity_column=Param(
            str, default="ellipticity_true", msg="column names for ellipticity"
        ),
    )
    inputs = [("input", PqHandle)]
    outputs = [("output", PqHandle)]
    ...
    def _get_AB(self, input_data):
        ...
        size = input_data[self.config.bulge_size_column] * f + input_data[
            self.config.disk_size_column
        ] * (1 - f)
        e = input_data[self.config.ellipticity_column]
        return compute_AB_from_size_ellip(size, e)
...
class MomentsToAB(CatalogManipulator):
    ...
    config_options = CatalogManipulator.config_options.copy()
    config_options.update(
        xx_column=Param(str, default="shape_xx", msg="column names for moment xx"),
        xy_column=Param(str, default="shape_xy", msg="column names for moment xy"),
        yy_column=Param(str, default="shape_yy", msg="column names for moment yy"),
    )
    inputs = [("input", PqHandle)]
    outputs = [("output", PqHandle)]
